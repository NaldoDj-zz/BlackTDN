    ...
    $MC=Measure-Command {    
        ...
        foreach ($Filial in $Filiais) {
            ...
            foreach ($Canal in $Canais.Values | Sort-Object )
            {
                ...
                $sortFile | ? {$_.Substring(0,4).Contains($Filial) -and "SRF".Contains($Canal) } | ? {
                        ...
                        if ($key -eq $lKey){
                            ...
                            $FilePath="$($pathOutFile)dupl-srf-$($Filial.Replace(' ','b'))-$($Canal).txt"
                        } else {
                            #Define o arquivo para dados OK
                            $FilePath="$($pathOutFile)srf-$($Filial.Replace(' ','b'))-$($Canal).txt"
                        }
