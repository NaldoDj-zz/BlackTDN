...
METHOD SetTReport() CLASS uTCREPORT
    ...
    IF (IsInCallStack("PRINTDIALOG").and.IsInCallStack("PRINT"))
        ...
        //-------------------------------------------------------------------------------------
            //...//acesso para alterar configuracao de impressao
        //-------------------------------------------------------------------------------------
        Self:lEdit:=(Self:GetEdit().and.VerSenha(110))
    ...
    Local __Dummy
       ...
       IF (__nOpcRpt==RPT_TREPORT)
        ...
        BEGIN SEQUENCE
            ...
            IF ((cFimFolha$"FP").or.(lIsInBreak))
                IF .NOT.(__oTCPrint:nLine==0)
                    IF ((cFimFolha$"F").or..NOT.(cRoda==NIL))
                        IF .NOT.(lbRoda)
                            __Roda(__nOpcRpt)
                        Else
                            Eval(bRoda)
