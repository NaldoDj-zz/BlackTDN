...
/*
   ...
   Descricao:Importacao de Movimento Mensal Via CSV (Arquivo Delimitado por ";").
              O arquivo devera ter cabecalho com as seguintes informacoes:
              1 ) RC_FILIAL
              2 ) RC_MAT
              3 ) RC_PD
              4 ) RC_TIPO1
...
User Function btdnGM01()
   ...
   Local cDescri:= OemToAnsi( "Este Programa ira efetuar a Importacao de arquivo da Folha de Pagamento (Movimento Mensal) conforme arquivo Selecionado" )
   ...
   Private cCadastro:= OemtoAnsi( "Importacao de Arquivos Folha de Pagamento:Movimento Mensal" )
...
Static Function btdnGM01(oProcess,oPanel)
   ...
   oProcess:SaveLog( OemToAnsi( "Inicio da Importacao" ) )
   ...
   RECOVER
      ...
      IF ( ValType( oError ) == "O" )
         ...
         IF MsgNoYes( "Ocorreram Erros Durante o Processo de Importacao. Deseja Visualizar?" , "AVISO!!!" )
            fMakeLog( { aError } , { "Log de Ocorrencias na Importacao do arquivo:" + cFile } , "" , .T. , NIL , cCadastro , NIL , NIL , NIL , .F. )  
         EndIF   
      EndIF
   ...
   oProcess:SaveLog( OemToAnsi( "Final da Importacao" ) )
...
Static Function ImportCSVExcel(oProcess,oPanel,cFile)
   ...
   IF Empty( cFile )
      UserException( GetHelp("NOFLEIMPOR") + " " + OemToAnsi( "Arquivo:" + cFile + " " + "Nao Encontrado" )  )
   EndIF
   ...
   IF ( nfLastRec == 0 )
      ft_fUse()
      UserException( "Nao existem informacoes a serem importadas para o arquivo:" + cFile )
   EndIF
   ...
   IF Empty( aHeader )
      ft_fUse()
      UserException( "Estrutura do arquivo:" + cFile + " Invalida. O Arquivo nao sera importado" )
   EndIF
   ...
   While .NOT.( ft_fEof() )
      ...
      BEGIN SEQUENCE
         ...
         IF SRA->( .NOT.( dbSeek( cSRAKeySeek , .F. ) ) )
            UserException( "Filial " + cSRAFilial + " e Matricula:" + cRCMAT + " Nao Cadastradas no Sistema. Registro " + LTrim(Str(ft_fRecno())) + " nao sera importado" )
         EndIF
         ...
         IF SRV->( .NOT.( dbSeek( cSRVKeySeek , .F. ) ) )
            UserException( "Filial " + cSRVFilial + " e Verba:" + cRCPD + " Nao Cadastradas no Sistema. Registro " + LTrim(Str(ft_fRecno())) + " nao sera importado" )
         EndIF
         
         IF ( cRCTIPO1 $ "D/H" )
            IF Empty( nRCHORAS )
               UserException( "Filial " + cSRAFilial + " e Matricula:" + cRCMAT + " Verba sem Quantidade' Informada. Registro " + LTrim(Str(ft_fRecno())) + " nao sera importado" )
            EndIF
         EndIF
         IF ( cRCTIPO1 $ "V" )
            IF Empty( nRCVALOR )
               UserException( "Filial " + cSRAFilial + " e Matricula:" + cRCMAT + " Verba sem Valor Informado. Registro " + LTrim(Str(ft_fRecno())) + " nao sera importado" )
            EndIF
         EndIF
         ...
         IF CTT->( .NOT.( dbSeek( cCTTKeySeek , .F. ) ) )
            UserException( "Filial " + cCTTFilial + " e Centro de Custo:" + cRCCC + " Nao Cadastrados no Sistema. Registro " + LTrim(Str(ft_fRecno())) + " nao sera importado" )
         EndIF
         ...
         cSRCKeySeek += cRCPD
            ...
            IF SRC->( dbSeek( cSRCKeySeek , .F. ) )
             UserException( "Filial " + cSRCFilial + " e Matricula:" + cRCMAT + " Ja Existe Lancamento na Verba:"+cRCPD+" para o Funcionario. Registro " + LTrim(Str(ft_fRecno())) + " nao sera importado" )
          EndIF
