...
/*
	Programa	: u_btdnGR01.PRW
	Funcao		: u_btdnGR01()
	Autor		: Marinaldo de Jesus [http://www.blacktdn.com.br]
	Data		: 17/06/2013
	Descricao	: Gerar Informacoes em Excel
*/                          
User Function btdnGR01()

	Local aPerg			:= Array(0)
	Local nPerg
	
	Local cFunction		:= "btdnGR01"
	Local cTitle		:= OemToAnsi( "Funcionários por Centro de Custo" )
	Local cDescription	:= cTitle
	
	Local bProcess		:= {|othis,oPanel|btdnGR01Excel(@lIsBlind,@othis,@oPanel,@cTitle)}
	...
	BEGIN SEQUENCE
		...
		Else
			...
			btdnGR01Prt(lIsBlind,MV_PAR01,cDescription)
		EndIF	
...
/*
	Programa	: u_btdnGR01.PRW
	Funcao		: btdnGR01Excel
	Autor		: Marinaldo de Jesus [http://www.blacktdn.com.br]
	Data		: 17/06/2013
	Descricao	: Gerar Informacoes em Excel
*/                          
Static Function btdnGR01Excel(lIsBlind,othis,oPanel,cTitle)
				  ...
				  AND NOT EXISTS (
									SELECT 
										...
										FROM 
											%table:SRE% SRE
										WHERE SRE.%NotDel%
										  AND SRE.RE_EMPP=%exp:cEmpAnt% 
										  AND SRE.RE_FILIALP=SRA.RA_FILIAL 
										  AND SRE.RE_MATP=SRA.RA_MAT 
										  AND SUBSTRING(SRE.RE_DATA,1,6)=%exp:cYearMonth% 
					)
					AND EXISTS(
									SELECT 
					  						DISTINCT 1 
...
*/
Static Function btdnGR01Prt(lIsBlind,cYearMonth,cDescription)
...
Static Function RunReport(lIsBlind,aNiveis,nNiveis)
	...
	BEGIN SEQUENCE
		...
		For nNivel := 1 To nNiveis
			...
			aAdd( aCabec1 , { 0 , "Filial: " + ( cAlias )->(FieldGet(nQField)) , GetSx3Cache("RA_FILIAL","X3_TAMANHO") } )
			aAdd( aCabec2 , { 0 , "" , 100 } )        
