...
/*
	Programa	: u_btdnGR06.PRW
	Funcao		: u_btdnGR06()
	Autor		: Marinaldo de Jesus [http://www.blacktdn.com.br]
	Data		: 17/06/2013
	Descricao	: Gerar Informacoes em Excel
*/                          
User Function btdnGR06()

	Local aPerg			:= Array(0)
	Local nPerg
	
	Local cFunction		:= "btdnGR06"
	Local cTitle		:= OemToAnsi( "Verbas 708/710" )
	Local cDescription	:= cTitle
	
	Local bProcess		:= {|othis,oPanel|btdnGR06Excel(@lIsBlind,@othis,@oPanel,@cTitle)}
	...
	BEGIN SEQUENCE
		...
		Else
			...
			btdnGR06Prt(lIsBlind,MV_PAR01,cDescription)
		EndIF	
...
/*
	Programa	: u_btdnGR06.PRW
	Funcao		: btdnGR06Excel
	Autor		: Marinaldo de Jesus [http://www.blacktdn.com.br]
	Data		: 17/06/2013
	Descricao	: Gerar Informacoes em Excel
*/                          
Static Function btdnGR06Excel(lIsBlind,othis,oPanel,cTitle)
		...
		BEGINSQL ALIAS cAlias
			...
			(
				...
				WHERE SRD.%NotDel%
				  ...
				  AND SRV.%NotDel%
				  AND CTT.%NotDel%
				  AND SRD.RD_FILIAL BETWEEN ' ' AND 'Z'
...
Static Function DBToExcel(othis,cAlias,nQryCTotal,lIsBlind,cYearMonth,cTable,lTotalize,lPicture)
	...
	Local nFBreak		:= (cAlias)->( FieldPos( "EMPRESA" ) )
	Local lBreak
...
*/
Static Function btdnGR06Prt(lIsBlind,cYearMonth,cDescription)
...
Static Function RunReport(lIsBlind,aNiveis,nNiveis)
	...
	BEGIN SEQUENCE
		...
		For nNivel := 1 To nNiveis
			...
			aAdd( aCabec1 , { 0 , { || "Empresa: " + ( cAlias )->(FieldGet(nQField)) } , 100 } )
			aAdd( aCabec2 , { 0 , "" , 100 } )        
