...
/*
	Programa	: u_btdnGR12.PRW
	Funcao		: u_btdnGR12()
	Autor		: Marinaldo de Jesus [http://www.blacktdn.com.br]
	Data		: 17/06/2013
	Descricao	: Gerar Informacoes em Excel informações pagamento Beneficiária Pensão Alimentícia (Diferença de Férias)
*/                          
User Function btdnGR12()

	Local aInfo
	Local aPerg			:= Array(0)
	Local nPerg
	
	Local cFunction		:= "btdnGR12"
	Local cTitle		:= OemToAnsi( "Pagamento a Pensionistas (Diferença de Férias)" )
	Local cDescription	:= cTitle
	
	Local bProcess		:= {|othis,oPanel|btdnGR12Excel(@lIsBlind,@othis,@oPanel,@cTitle)}
	...
	BEGIN SEQUENCE
		...
		Else
			...
			btdnGR12Prt(@lIsBlind,@MV_PAR01,@cDescription)
		EndIF	
...
/*
	Programa	: u_btdnGR12.PRW
	Funcao		: btdnGR12Excel
	Autor		: Marinaldo de Jesus [http://www.blacktdn.com.br]
	Data		: 17/06/2013
	Descricao	: Gerar Informacoes em Excel 
*/                          
Static Function btdnGR12Excel(lIsBlind,othis,oPanel,cTitle)
...
Static Function QueryView(cYearMonth,nQryCTotal,cdVencimento)
	...
	BEGIN SEQUENCE 
		...
		nfh 	:= fCreate(cDrive+"\gerador\"+cEmpAnt+"_Pagamento_a_Pensionistas_Diferenca_de_Ferias_"+cYearMonth+".txt")
		lfh		:= ( nfh <> -1 )
		...
		While (cAlias)->( .NOT.( Eof() ) )
			...
			cLine += __cTabC
			cLine += __cTabC
			cLine +=  "PENSAO ALIMENTICIA DIFERENCA DE FERIAS: " + Transform( cYearMonth , "@E 9999/99")
			cLine +=  "-"
...
*/
Static Function btdnGR12Prt(lIsBlind,cYearMonth,cDescription)
