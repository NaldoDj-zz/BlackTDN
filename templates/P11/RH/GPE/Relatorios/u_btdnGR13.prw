...
/*
	Programa	: u_btdnGR13.PRW
	Funcao		: u_btdnGR13()
	Autor		: Marinaldo de Jesus [http://www.blacktdn.com.br]
	Data		: 17/06/2013
	Descricao	: Gerar Informacoes em Excel informações pagamento Beneficiária Pensão Alimentícia (2a.Parcela 13o.Salario)
*/                          
User Function btdnGR13()

	Local aInfo
	Local aPerg			:= Array(0)
	Local nPerg
	
	Local cFunction		:= "btdnGR13"
	Local cTitle		:= OemToAnsi( "Pagamento a Pensionistas (2a.Parcela 13o.Salario)" )
	Local cDescription	:= cTitle
	
	Local bProcess		:= {|othis,oPanel|btdnGR13Excel(@lIsBlind,@othis,@oPanel,@cTitle)}
	...
	BEGIN SEQUENCE
		...
		Else
			...
			btdnGR13Prt(@lIsBlind,@MV_PAR01,@cDescription)
		EndIF	
...
/*
	Programa	: u_btdnGR13.PRW
	Funcao		: btdnGR13Excel
	Autor		: Marinaldo de Jesus [http://www.blacktdn.com.br]
	Data		: 17/06/2013
	Descricao	: Gerar Informacoes em Excel
*/                          
Static Function btdnGR13Excel(lIsBlind,othis,oPanel,cTitle)
...
Static Function QueryView(cYearMonth,nQryCTotal,cdVencimento)
	...
	Local lUseSRD		:= MsFile( "RI"+cEmpAnt+SUBSTR(cYearMonth,3) )
	...
	BEGIN SEQUENCE 
			...
			BEGINSQL ALIAS cAlias
				...
				(
					SELECT
						...
						,SRA.RA_MAT						 	AS CODIGO
						,SRA.RA_NOME					 	AS FUNCIONARIO
						,SRQ.RQ_ORDEM						AS ORDEM
						,SRQ.RQ_NOME				     	AS FORNECEDOR
						,%exp:cYearMonth%				 	AS COMPET
						,%exp:cdVencimento%				 	AS VENCIMENTO
						,(SRQ.RQ_BCDEPBE+SRQ.RQ_CTDEPBE) 	AS NUMERO_CONTROLE
						,SUM(SRI.RI_VALOR)				 	AS VALOR
					   	,%exp:cSQLReplace%					AS EMBARCACOES
					   	,SRQ.RQ_XCODSAP						AS CODIGO_SAP
					FROM
						  %table:SRA% SRA
					...
					WHERE SRA.%notDel%
					  ...
					  AND SRQ.%notDel% 
					  AND SRQ.RQ_SEQUENC='01'
					  AND SRA.RA_FILIAL=SRI.RI_FILIAL
					  AND SRA.RA_MAT=SRI.RI_MAT
					  AND SRI.%notDel% 
					  AND SRI.RI_PD IN ( %exp:cSQLPDPen% )
				 	  AND CTT.%notDel%
					  AND CTT.CTT_FILIAL BETWEEN ' ' AND 'Z'
					  AND SRA.RA_CC=CTT.CTT_CUSTO
					  AND SRI.RI_PD=RQ_VERB132
               		  AND NOT EXISTS (
               							SELECT 
               								DISTINCT 1
               							FROM
		...
		nfh 	:= fCreate(cDrive+"\gerador\"+cEmpAnt+"_Pagamento_a_Pensionistas_2a_Parcela_13o_Salario_"+cYearMonth+".txt")
		lfh     := ( nfh <> -1 )
		...
		While (cAlias)->( .NOT.( Eof() ) )
			...
			cLine += __cTabC
			cLine += __cTabC
			cLine +=  "PENSAO ALIMENTICIA 2a.PARCELA 13o.SALARIO: " + Transform( cYearMonth , "@E 9999/99")
			cLine +=  "-"
...
*/
Static Function btdnGR13Prt(lIsBlind,cYearMonth,cDescription)
