...
/*
    Programa    : u_btdnGR15.PRW
    Funcao        : u_btdnGR15()
    Autor        : Marinaldo de Jesus [http://www.blacktdn.com.br]
    Data        : 17/06/2013
    Descricao    : Gerar Informacoes em Excel informações pagamento Beneficiária Pensão Alimentícia (Férias)
*/                          
User Function btdnGR15()

    Local aInfo
    Local aPerg            := Array(0)
    Local nPerg
    
    Local cFunction        := "btdnGR15"
    Local cTitle        := OemToAnsi( "Pagamento a Pensionistas (Férias)" )
    Local cDescription    := cTitle
    
    Local bProcess        := {|othis,oPanel|btdnGR15Excel(@lIsBlind,@othis,@oPanel,@cTitle)}
    ...
    BEGIN SEQUENCE
        ...
        Else
            ...
            btdnGR15Prt(@lIsBlind,@MV_PAR01,@cDescription)
        EndIF    
...
/*
    Programa    : u_btdnGR15.PRW
    Funcao        : btdnGR15Excel
    Autor        : Marinaldo de Jesus [http://www.blacktdn.com.br]
    Data        : 17/06/2013
    Descricao    : Gerar Informacoes em Excel
*/                          
Static Function btdnGR15Excel(lIsBlind,othis,oPanel,cTitle)
...
Static Function QueryView(cYearMonth,nQryCTotal,cdVencimento)
    ...
    BEGIN SEQUENCE 
                      ...
                      AND SRR.RR_TIPO3='F'
                         AND NOT EXISTS (
                                           ...
                                           FROM
                                               %table:SRR% SRR_R
                                           WHERE SRR_R.%notDel%
                                             AND SRR_R.RR_FILIAL=SRA.RA_FILIAL
                                             AND SRR_R.RR_MAT=SRA.RA_MAT
                                             AND SRR_R.RR_PD IN ( %exp:cSQLPDPen% )
                                             AND SRR_R.RR_TIPO3='R'
                       )
                    GROUP BY
                         SRA.RA_FILIAL
                        ,SRA.RA_MAT
        ...
        nfh     := fCreate(cDrive+"\gerador\"+cEmpAnt+"_Pagamento_a_Pensionistas_Ferias_"+cYearMonth+".txt")
        lfh        := ( nfh <> -1 )
        ...
        While (cAlias)->( .NOT.( Eof() ) )
            ...
            cLine += __cTabC
            cLine += __cTabC
            cLine +=  "PENSAO ALIMENTICIA FERIAS: " + Transform( cYearMonth , "@E 9999/99")
            cLine +=  "-"
...
*/
Static Function btdnGR15Prt(lIsBlind,cYearMonth,cDescription)
