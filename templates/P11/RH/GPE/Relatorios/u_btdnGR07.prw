...
User Function GPER040X()
    ...
    BEGIN SEQUENCE
        ...
        IF .NOT.(lGPER040)
            APMsgAlert(OemToAnsi("Chamada INVALIDA"),OemToAnsi("ATENCAO"))
            BREAK
        EndIF
        ...
        IF .NOT.(Type("aPVGper040")=="A")
            APMsgAlert(OemToAnsi("Ocorreram Problemas durante a Configuracao de Impressao. Contate o Administrador do Sistema"),OemToAnsi("ATENCAO"))
            lRet:=.F.
            BREAK
        EndIF

        IF .NOT.(nOR(1,2,3)==nOr(__nOrdem,6)) //1:RA_FILIAL+RA_CC+RA_MAT;2:RA_FILIAL+RA_MAT;3:RA_FILIAL+RA_NOME
            APMsgAlert(OemToAnsi("Ordem de Impressao INVALIDA. Selecione por:C.Custo Cadastro; Matricula ; ou Nome"),OemToAnsi("ATENCAO"))
            lRet:=.F.
            BREAK
        EndIF

        IF .NOT.(__aReturn[5]==2) //Via SPOOL
            APMsgAlert(OemToAnsi("Tipo de Impressao nao Permitida. Selecione Tipo Impressao:Via Spool"),OemToAnsi("ATENCAO"))
            lRet:=.F.
            BREAK
        EndIF

        IF .NOT.("PDFCREATOR" $ Upper(__cPrtSelected)) //Verifica a Impressora Selecionada
            APMsgAlert(OemToAnsi("Impressora INVALIDA. Em Opcoes selecione:PDFCreator"),OemToAnsi("ATENCAO"))
            lRet:=.F.
            BREAK
        EndIF
        ...
        lServer:=(__nAmbiente==AMB_SERVER)
           IF (lServer) //Verifica se a impressao sera no server
            APMsgAlert(OemToAnsi("Impressao no servidor nao permitida. Selecione a impressao no Cliente"),OemToAnsi("ATENCAO"))
            lRet:=.F.
            BREAK
        EndIF
...
Static Function btdnGR07(lEnd,lProcessa,cTitulo)
    ...
    BEGIN SEQUENCE
        ...
        IF .NOT.(__lIsBlind)
            lSendMail:=MsgYesNo(OemToAnsi("Geracao Finalizada. Deseja enviar os arquivos por e-mail?"),OemToAnsi("ATENCAO"))
            IF .NOT.(lSendMail)
                BREAK
            EndIF
        EndIF
            ...
            IF File(cPDFFile)
                ...
                IF (lMailOK)
                    ...
                    /*
                        ...
                        MV_RELPSW     //Senha para autenticacao no servidor de e-mail
...
Static Function GetStackParameters()
    ...
    RECOVER
        ...
        EndIF
        aStackEnv:=StrTokArr2(cStackEnv,CRLF)
