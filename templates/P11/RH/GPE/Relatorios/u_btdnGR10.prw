...
/*
	Programa	: u_btdnGR10.PRW
	Funcao		: u_btdnGR10()
	Autor		: Marinaldo de Jesus [http://www.blacktdn.com.br]
	Data		: 17/06/2013
	Descricao	: Gerar Informacoes em Excel informações pagamento Sindical
*/                          
User Function btdnGR10()

	Local aInfo
	Local aPerg			:= Array(0)
	Local nPerg
	
	Local cFunction		:= "btdnGR10"
	Local cTitle		:= OemToAnsi( "Contribuição Sindical (Verba 417)" )
	Local cDescription	:= cTitle
	
	Local bProcess		:= {|othis,oPanel|btdnGR10Excel(@lIsBlind,@othis,@oPanel,@cTitle)}
	...
	BEGIN SEQUENCE
		...
		Else
			...
			btdnGR10Prt(@lIsBlind,@MV_PAR01,@cDescription)
		EndIF	
...
/*
	Programa	: u_btdnGR10.PRW
	Funcao		: btdnGR10Excel
	Autor		: Marinaldo de Jesus [http://www.blacktdn.com.br]
	Data		: 17/06/2013
	Descricao	: Gerar Informacoes em Excel
*/                          
Static Function btdnGR10Excel(lIsBlind,othis,oPanel,cTitle)
...
Static Function QueryView(cYearMonth,nQryCTotal,cdVencimento)
	...
	BEGIN SEQUENCE
		...
		BEGINSQL ALIAS cAlias
			...
			%noparser%
			SELECT  (RCE.RCE_CODIGO+'-'+RCE.RCE_DESCRI)	AS FORNECEDOR
				   ,SRD.RD_DATARQ						AS EMISSAO
				   ,%exp:cdVencimento% 					AS VENCIMENTO
				   ,RCE.RCE_FAX							AS NF
				   ,SUM(SRD.RD_VALOR)   				AS RD_VALOR
				   ,(SRA.RA_CC+'-'+CTT.CTT_DESC01)		AS EMBARCACOES
				   ,RCE.RCE_XCODSA						AS COD_SAP 
			FROM  %table:SRA% SRA
				,%table:CTT% CTT
			    ,%table:SRD% SRD
			    ,%table:RCE% RCE
			WHERE SRA.%notDel%
			  ...
			  AND RCE.RCE_FILIAL BETWEEN ' ' AND 'Z'
			  AND SRA.RA_SINDICA=RCE.RCE_CODIGO
			GROUP BY
			    RCE.RCE_CODIGO
			   ,RCE.RCE_DESCRI
			   ,SRD.RD_DATARQ
		...
		nfh 	:= fCreate(cDrive+"\gerador\"+cEmpAnt+"_contribuicao-sindical_"+cYearMonth+".txt")
		lfh		:= ( nfh <> -1 )
		...
		While (cAlias)->( .NOT.( Eof() ) )
			...
			cLine += "BRL"
			cLine += __cTabC
			cLine += (cAlias)->( EMBARCACOES )
			cLine += __cTabC
			cLine += "40"
			cLine += __cTabC
			cLine += "214204"
			cLine += __cTabC
			cLine += (cAlias)->( AllTrim( Transform( RD_VALOR , "@E 999,999,999.99" ) ) )
			cLine += __cTabC
			cLine += __cTabC
			cLine += __cTabC
			cLine += __cTabC
			cLine +=  "CONTRIBUICAO SINDICAL"
			cLine +=  "-"
...
Static Function DBToExcel(othis,cAlias,nQryCTotal,lIsBlind,cYearMonth,cTable,lTotalize,lPicture)
	...
	Local nFBreak		:= (cAlias)->( FieldPos( "FORNECEDOR" ) )
	Local lBreak
...
*/
Static Function btdnGR10Prt(lIsBlind,cYearMonth,cDescription)
...
Static Function RunReport(lIsBlind,aNiveis,nNiveis)
	...
	BEGIN SEQUENCE
		...
		For nNivel := 1 To nNiveis
			...
			aAdd( aCabec1 , { 0 , { || "Fornecedor: " + ( cAlias )->(FieldGet(nQField)) } , 100 } )
			aAdd( aCabec2 , { 0 , "" , 100 } )        
