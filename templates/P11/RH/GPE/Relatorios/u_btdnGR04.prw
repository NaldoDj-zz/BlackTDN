...
/*
    Programa    : u_btdnGR04.PRW
    Funcao        : u_btdnGR04()
    Autor        : Marinaldo de Jesus [http://www.blacktdn.com.br]
    Data        : 17/06/2013
    Descricao    : Gerar Informacoes em Excel
*/                          
User Function btdnGR04()

    Local aPerg            := Array(0)
    Local nPerg
    
    Local cFunction        := "btdnGR04"
    Local cTitle        := OemToAnsi( "Resumo de Imposto de Renda por Centro de Custo" )
    Local cDescription    := cTitle
    
    Local bProcess        := {|othis,oPanel|btdnGR04Excel(@lIsBlind,@othis,@oPanel,@cTitle)}
    ...
    BEGIN SEQUENCE
        ...
        Private lSintetico    := IF( lIsBlind , .T. , MsgYesNo( "Gerar em Modo Sintético?" , "Atenção" ) )
        
        IF .NOT.( lSintetico )
            aAdd( aNotPrint , "CTT_DESC01" )
        EndIF
        aAdd( aNotPrint , "ORIGEM" )
        aAdd( aNotPrint , "QORIGEMCC" )
        ...
        Else
            ...
            btdnGR04Prt(lIsBlind,MV_PAR01,cDescription)
        EndIF    
...
/*
    Programa    : u_btdnGR04.PRW
    Funcao        : btdnGR04Excel
    Autor        : Marinaldo de Jesus [http://www.blacktdn.com.br]
    Data        : 17/06/2013
    Descricao    : Gerar Informacoes em Excel
*/                          
Static Function btdnGR04Excel(lIsBlind,othis,oPanel,cTitle)
...
Static Function QueryView(cYearMonth,nQryCTotal)
    ...
    Local lAnalitico    := ( ( Type("lSintetico") == "L" ) .and. .NOT.(lSintetico) )
...
Static Function DBToExcel(othis,cAlias,nQryCTotal,lIsBlind,cYearMonth,cTable,lTotalize,lPicture)
    ...
    Local nFBreak        := (cAlias)->( FieldPos( IF( lAnalitico , "QORIGEMCC" , "ORIGEM" ) ) )
    Local lBreak
...
*/
Static Function btdnGR04Prt(lIsBlind,cYearMonth,cDescription)
...
Static Function RunReport(lIsBlind,aNiveis,nNiveis)
    ...
    Local cQField        := IF( lAnalitico , "CTT_DESC01" , "ORIGEM" )
    ...
    BEGIN SEQUENCE
        ...
        For nNivel := 1 To nNiveis
            ...
            aAdd( aCabec1 , { 0 , ( cAlias )->(FieldGet(nQField)) , 100 } )
            aAdd( aCabec2 , { 0 , "" , 100 } )        
