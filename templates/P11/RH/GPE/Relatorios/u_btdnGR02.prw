...
/*
    Programa    : u_btdnGR02.PRW
    Funcao        : u_btdnGR02()
    Autor        : Marinaldo de Jesus [http://www.blacktdn.com.br]
    Data        : 17/06/2013
    Descricao    : Gerar Informacoes em Excel
*/                          
User Function btdnGR02()

    Local aPerg            := Array(0)
    Local nPerg
    
    Local cFunction        := "btdnGR02"
    Local cTitle        := OemToAnsi( "Funcionários por Centro de Custo" )
    Local cDescription    := cTitle
    
    Local bProcess        := {|othis,oPanel|btdnGR02Excel(@lIsBlind,@othis,@oPanel,@cTitle)}
    ...
    Local nCTTXXCONT    := CTT->( FieldPos( "CTT_XXCONT" ) )
    Local nCTTXXCONU    := CTT->( FieldPos( "CTT_XXCONU" ) )
    
    Private cCadastro     := cTitle
    
    BEGIN SEQUENCE 
    
        IF ( ( nCTTXXCONT == 0 ) .or. ( nCTTXXCONU == 0 ) )
            ApMsgAlert( "Opção nao Disponível. Faltam os campos: CTT_XXCONT e CTT_XXCONU" , "ATENÇÃO!!!" )
            BREAK
        EndIF
        ...
        aAdd( aNotPrint , "CTT_XXCONT" )
        aAdd( aNotPrint , "CTT_XXCONU" )

        Private aFRosto        := Array(0)
    
        aAdd( aFRosto , '"À"' )
        aAdd( aFRosto , '" "' )
        aAdd( aFRosto , '"Acessoria Deloite"' )
        aAdd( aFRosto , '" "' )
        aAdd( aFRosto , '"Vimos por meio desta encaminhar, conforme solicitado, a relação de funcionários alocados na"' )
        aAdd( aFRosto , '"PETROBRÁS S/A desenvolvendo atividades para:"' )
        aAdd( aFRosto , '" "' )
        aAdd( aFRosto , '"COMPETÊNCIA: "+"'+( SubStr(MV_PAR01,-2)+"/"+SubStr(MV_PAR01,1,4))+'"' )
        aAdd( aFRosto , '"C.CUSTO: "+FieldGet(FieldPos("CTT_DESC01"))' )
        aAdd( aFRosto , '"CONTRATO: "+FieldGet(FieldPos("CTT_XXCONT"))' )
        aAdd( aFRosto , '"UNIDADE: "+FieldGet(FieldPos("CTT_XXCONU"))' )
        ...
        Else
            ...
            btdnGR02Prt(lIsBlind,MV_PAR01,cDescription)
        EndIF    
...
/*
    Programa    : u_btdnGR02.PRW
    Funcao        : btdnGR02Excel
    Autor        : Marinaldo de Jesus [http://www.blacktdn.com.br]
    Data        : 17/06/2013
    Descricao    : Gerar Informacoes em Excel
*/                          
Static Function btdnGR02Excel(lIsBlind,othis,oPanel,cTitle)
...
Static Function QueryView(cYearMonth,nQryCTotal)
    ...
    BEGIN SEQUENCE
        ...
        BEGINSQL ALIAS cAlias
            ...
            COLUMN RA_SALARIO AS NUMERIC(15,2)
            %noparser%
            SELECT 
                x.*
            FROM (
                SELECT 
                    ...
                    ,SRA.RA_ADMISSA
                    ,SRJ.RJ_DESC
                    ,ISNULL(SRD.RD_VALOR,SRA.RA_SALARIO) RA_SALARIO
                    ,CTT.CTT_DESC01
                    ,CTT.CTT_XXCONT
                    ,CTT.CTT_XXCONU
                FROM %table:SRA% SRA
...
Static Function DBToExcel(othis,cAlias,nQryCTotal,lIsBlind,cYearMonth,cTable,lTotalize,lPicture)
    ...
    Local lFRosto        := ( Type("aFRosto") == "A" .and. ( Len( aFRosto ) > 0 ) )
    
    Local nRow
    Local nRows
    
    DEFAULT cWorkSheet    := "btdnGR02"
        ...
        While (cAlias)->( .NOT.( Eof() ) )
            ...
            IF ( lBreak )
                ...
                IF ( lFRosto )
                    ...
                    cWBreak := "CAPA" + " - " + cLBreak
                    cTBreak := cTable
                        
                    oFWMSExcel:AddworkSheet(cWBreak)
                    oFWMSExcel:AddTable(cWBreak,cTBreak)
            
                    oFWMSExcel:AddColumn(@cWBreak,@cTBreak,@cColumn,@nAlign,@nFormat,.F.)
                
                    For nRow := 1 TO nRows
                        uCell        := aFRosto[nRow]
                        uCell        := (cAlias)->( &uCell )
                        aCells[1]    := OemToAnsi( uCell )
                ...
                For nField := 1 To nFields
                    ...
                    ElseIF ( "RA_SALARIO" $ aHeader[nField][DBS_NAME] )
                        cColumn    := "Soldada Base"
                    EndIF
...
*/
Static Function btdnGR02Prt(lIsBlind,cYearMonth,cDescription)
    ...
    BEGIN SEQUENCE
        ...
        For nNivel := 1 To nNiveis
            ...
            For nField := 1 To nFields
                ...
                ElseIF ( "RA_SALARIO" $ aFields[nField][DBS_NAME] )
                    cX3Titulo                    := "Soldada Base"
                EndIF
            ...
            IF ( lFRosto )
                (cAlias)->(aEval(aFRosto,{|t|aAdd(aCabec1,{0,OemToAnsi(&t),220})}))
            EndIF
